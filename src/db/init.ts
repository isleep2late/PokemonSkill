...
const dbPath = path.resolve('data', 'PokemonSkill.db');
 ...
 *
 * The bot runs SQLite in WAL mode, so writes accumulate in PokemonSkill.db-wal
 * and are only merged into the main .db at checkpoints. If the process dies
 * without closing the connection, the WAL is left un-checkpointed and anything
 * reading the .db alone sees stale ratings. Checkpointing with TRUNCATE here
 * folds all pending writes back into the main file and empties the WAL, so a
 * clean stop always leaves a self-contained, up-to-date database.
 */
...
export async function initDatabase() {
  ...
  // Timewalk events table for persisting virtual decay events across recalculations
  await db.exec(`
    CREATE TABLE IF NOT EXISTS timewalk_events (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
