...
export async function execute(
  ...
  try {
    ...
    const dbPath = path.join(process.cwd(), 'data', 'PokemonSkill.db');
    ...
    const baseFilename = `PokemonSkill-backup-${dateStr}_${timeStr}`;
    ...
    await interaction.editReply({
      content: `⚠️ Couldn't send backup via DM (you may have DMs disabled).\n\n` +
               ...
               `• Size: ${fs.statSync(path.join(process.cwd(), 'data', 'PokemonSkill.db')).size / (1024 * 1024)}MB`
    });
  }
}
