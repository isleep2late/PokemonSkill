...
export const data = new SlashCommandBuilder()
  .setName('view')
  .setDescription('View league statistics, player stats, deck/team stats, or game details')
  .addStringOption(option =>
    option
      ...
      .addChoices(
        ...
        { name: 'Deck/Team', value: 'commander' },
        { name: 'Game', value: 'game' },
        { name: 'Win % (Players)', value: 'winpct_players' },
        { name: 'Win % (Decks/Teams)', value: 'winpct_commanders' }
      )
  )
  ...
  .addStringOption(option =>
    option.setName('commander')
      .setDescription('Deck or team name to view (for deck/team type)')
      .setRequired(false)
  )
  .addStringOption(option =>
...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  if (type === 'league') {
    ...
    if (!commanderName) {
      await interaction.editReply({
        content: '❌ You must specify a deck/team name when using type:commander.'
      });
      return;
    }
    ...
    if (!await deckExistsWithGames(normalizedName)) {
      await interaction.editReply({
        content: `❌ Deck/Team "${commanderName}" has not been played in any games yet.`
      });
      return;
    }
...
async function showLeagueStats(interaction: ChatInputCommandInteraction) {
  ...
  try {
    ...
    embed.addFields({
      name: '⚔️ Deck/Team Population',
      value: 
        `**Total Decks/Teams:** ${totalDecks}\n` +
        `**Qualified Decks/Teams:** ${qualifiedDecks.length} (≥5 games)\n` +
        `**Different Decks/Teams Used:** ${activeCommandersResult.count}\n` +
    ...
    if (topPlayer || topDeck) {
      ...
      if (topDeck) {
        ...
        topPerformersText += `**Top Deck/Team:** ${topDeck.displayName} (${topDeckElo} Elo)`;
      }
    ...
    if (topCommandersDisplay) {
      embed.addFields({
        name: '⚔️ Most Played Decks/Teams',
...
async function showCommanderStats(interaction: ChatInputCommandInteraction, commanderName: string) {
  ...
  try {
    ...
    if (!deck) {
      await interaction.editReply({
        content: `❌ Deck/Team "${commanderName}" not found in the database.`
      });
      return;
    }
    ...
    const embed = new EmbedBuilder()
      .setTitle(`⚔️ Deck/Team Statistics: ${deck.displayName}`)
      .setColor(qualified ? 0x00AE86 : 0xFFAA00);
    ...
    logger.error('Error fetching deck/team stats:', error);
    await interaction.editReply({
      content: '❌ Error fetching deck/team statistics.'
    });
  }
}
        ...
        for (const match of matches) {
          ...
          // CRITICAL FIX: Show assigned commander
            if (match.assignedDeck) {
              ...
              fieldValue += `**Deck/Team:** ${commanderDisplay}\n`;
            }
...
async function showCommanderWinPercentage(interaction: ChatInputCommandInteraction, count: number) {
  try {
    ...
    if (ranked.length === 0) {
      await interaction.editReply({ content: 'No decks/teams with games found.' });
      return;
    }
    ...
    const embed = new EmbedBuilder()
      .setTitle(`Top ${displayedCount} Decks/Teams by Win %`)
      .setColor(0x9B59B6)
      .setDescription(description)
      .setFooter({ text: `This is NOT the season ranking — win % does not reflect Elo or skill rating${truncatedNote}` })
      .setTimestamp();
    ...
    await interaction.editReply({
      content: '❌ An error occurred while generating deck/team win percentage statistics.'
    });
  }
}