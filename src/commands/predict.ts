// Predict 1v1 (Pokémon TCG Pocket / Showdown) win probabilities and view turn order stats.
...
export const data = new SlashCommandBuilder()
  .setName('predict')
  .setDescription('Predict 1v1 outcomes or view turn order statistics')
  .addStringOption(option =>
    option.setName('participants')
      .setDescription('1-2 participants: "@user,deck-name,phantom" - missing slot filled with a phantom')
      .setRequired(false)
  );
  ...
  try {
    ...
    if (entries.length === 0) {
      await interaction.editReply({
        content: '❌ No valid participants found in your input. Use format: "@user,deck-name,phantom" or combinations thereof.'
      });
      return;
    }

    // 1v1 — accept 1 or 2 participants; pad to 2 with a phantom if only one is given.
    if (entries.length > 2) {
      await interaction.editReply({
        content: '❌ Maximum 2 participants allowed for 1v1 predictions.'
      });
      return;
    }

    // Free-form deck/team names — `validateCommander` always returns true here.
    ...
    if (commanderNames.length > 0) {
      for (const commanderName of commanderNames) {
        if (!await validateCommander(commanderName)) {
          await interaction.editReply({
            content: `❌ "${commanderName}" is not a valid deck/team name format.`
          });
          return;
        }
      }
    ...
    // Pad with a phantom opponent if the user only supplied one participant.
    const paddedEntries = padEntriesTo2Players(entries);
...
function padEntriesTo2Players(entries: ParsedEntry[]): ParsedEntry[] {
  const paddedEntries = [...entries];

  // Add a phantom opponent with baseline (1000 Elo) for the missing 1v1 slot.
  for (let i = entries.length; i < 2; i++) {
    paddedEntries.push({
      type: 'player',
...
async function showTurnOrderStatistics(interaction: ChatInputCommandInteraction) {
  ...
  try {
    ...
    const description = results.map(r => {
      ...
      const turnEmoji = ['🥇', '🥈'][r.turnOrder - 1] || '❓';
      const positionLabel = r.turnOrder === 1 ? 'Went First' : r.turnOrder === 2 ? 'Went Second' : `Turn ${r.turnOrder}`;
      return `${turnEmoji} **${positionLabel}**: ${winRateStr}% (${r.wins}/${r.totalGames} games)`;
    }).join('\n');
...
async function getLeagueTurnOrderBaseline(): Promise<Record<number, number>> {
  ...
  // Convert to baseline win rates. 1v1 default is 50% per slot.
  const baseline: Record<number, number> = {};
  for (const [turnOrder, data] of turnData.entries()) {
    baseline[turnOrder] = data.total > 0 ? data.wins / data.total : 0.5;
  }

  // Ensure defaults for both 1v1 turn slots (1 = went first, 2 = went second).
  for (let i = 1; i <= 2; i++) {
    if (!(i in baseline)) {
      baseline[i] = 0.5;
    }
    ...
    entries.map(async entry => {
      ...
      if (entry.displayName.startsWith('Phantom Player')) {
        return {
          ...
          elo: 1000,
          turnOrderWinRate: leagueBaseline[entry.turnOrder] || 0.5,
          turnOrderGames: 0,
...
function calculateEnhancedTurnOrderPredictions(participants: ParticipantWithStats[], leagueBaseline: Record<number, number>): Array<{name: string, turnOrder: number, pct: number, personalWeight: number, turnOrderGames: number}> {
  const enhancedTurnResults = participants.map(p => {
    const leagueWinRate = leagueBaseline[p.turnOrder] || 0.5;
  ...
  return enhancedTurnResults
    .map(e => ({
      ...
      pct: totalBlendedRate > 0 ? Math.round((e.blendedRate / totalBlendedRate) * 100) : 50,
...
function calculateHybridPredictions(participants: ParticipantWithStats[], leagueBaseline: Record<number, number>): Array<{name: string, turnOrder: number, pct: number}> {
  ...
  return hybridResults
    .map(e => ({
      name: e.name,
      turnOrder: e.turnOrder,
      pct: totalHybridScore > 0 ? Math.round((e.hybridScore / totalHybridScore) * 100) : 50
    }))
    .sort((a, b) => b.pct - a.pct);
}
