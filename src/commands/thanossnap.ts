...
export async function execute(
  ...
  const dbFile = path.resolve('data', 'PokemonSkill.db');
      ...
      for (let i = 0; i < backupFiles.length; i += batchSize) {
        ...
        if (isFirstBatch) {
          ...
          if (backupFiles.length > 1) {
            ...
            messageContent += `3. Replace the current \`PokemonSkill.db\` with the reconstructed file\n`;
            messageContent += `4. Restart the bot\n\n`;
          } else {
            messageContent += `**Size**: ${fileSizeMB}MB\n\n`;
            messageContent += `**To restore this season:**\n`;
            messageContent += `1. Download the backup file\n`;
            messageContent += `2. Replace the current \`PokemonSkill.db\` with this backup\n`;
